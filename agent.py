...
{{
  "reasoning": "**Macro:** bullet points ONLY on macro-level factors — no company-specific items here:\\n• Risk regime: risk-on or risk-off signal (SPY/QQQ trend, VIX, credit spreads, safe-haven flows)\\n• Fed/rates: current Fed stance, recent FOMC/jobs/CPI/PCE data and what it means for equities\\n• Macro catalyst: any systemic event driving the market (geopolitics, tariffs, recession signals, liquidity conditions) — only include if material\\n**Sectors:** one bullet per sector with meaningful movement — reference the sector ETF and call out the specific return or theme:\\n• SMH (semis): leading/lagging vs QQQ by X%, key driver\\n• IGV (software): relative to semis, growth/value rotation read\\n• XLK (broad tech): overall posture\\n• XLY vs XLP: risk-on or risk-off consumer signal\\n• [add XLE, XLF, XLC, XLU, XLI, XLV only if they are notable movers or relevant to your positions]\\n• Omit sectors with no meaningful update\\n**Positions:** one bullet per held position that has a material update — skip positions with no notable news, earnings, or price action:\\n• TICKER (dir, +X.X% unreal): specific development — earnings result, news catalyst, technical level reached, stop proximity, or thesis change\\n• Only include positions worth flagging; do NOT list every position if nothing has changed\\n**Cash:** one sentence — why you hold this cash level and what would trigger deployment or further reduction",
  "actions": [
    {{
...
Return ONLY valid JSON — no prose, no markdown fences, no text outside the JSON object.
{{
  "reasoning": "**Macro:** bullet points ONLY on macro-level factors — no company-specific items here:\\n• Risk regime: risk-on or risk-off signal (equity trend, VIX, credit spreads, safe-haven flows)\\n• Fed/rates: current Fed stance, rate path expectations, recent FOMC/CPI/PCE/jobs data and long-term implications\\n• Growth cycle: GDP trend, PMI, consumer confidence — recession or expansion signals\\n• Macro catalyst: any systemic factor driving long-term capital allocation (AI capex cycle, energy transition, fiscal policy) — only if material\\n**Sectors:** one bullet per sector with meaningful movement — reference the sector ETF and call out the specific return or theme:\\n• SMH (semis): leading/lagging vs QQQ, AI capex cycle read\\n• IGV (software): relative to semis, SaaS multiples, growth/value rotation\\n• XLK (broad tech): overall posture\\n• XLY vs XLP: risk-on or risk-off consumer signal\\n• [add XLE, XLF, XLC, XLU, XLI, XLV only if they are notable movers or relevant to your positions]\\n• Omit sectors with no meaningful update\\n**Positions:** one bullet per held position that has a material update — skip positions with no notable news, earnings, or price action:\\n• TICKER (dir, +X.X% unreal): specific development — earnings result, news catalyst, thesis check, stop proximity, trim or add consideration\\n• Only include positions worth flagging; do NOT list every position if nothing has changed\\n**Cash:** one sentence — current cash level and whether you are waiting for better entry points, macro clarity, or are fully deployed by conviction",
  "actions": [
    {{
      "action": "BUY" | "SELL",
      "ticker": "NVDA",

...
| **Stop losses** | Optional — Claude sets at its discretion | Optional — Claude sets at its discretion |
...
4.  Fetch fundamental data for all 145 tickers (P/E, revenue growth, margins, D/E, ROE)
5.  Fetch recent news headlines + upcoming earnings for all 145 watchlist tickers
6.  Load peer agent's holdings.json (for coordination context)
7.  Enforce mechanical stop-losses (before Claude is called)
8.  Accrue daily borrow cost on short positions (swing only)
9.  Call Claude API with: memory + portfolio state + technicals + fundamentals + news + earnings + peer holdings
10. Execute Claude's trade decisions (with hard constraint checks)
11. Recompute performance metrics
12. Update memory.json with this run's reasoning + memory_update
13. Save holdings.json (atomic write)
14. Append to equity_log.jsonl (one line: timestamp + portfolio value)
15. Append to trade_log.md
16. Overwrite summary.md
```

Steps 7 and 8 are enforced in Python — Claude cannot override hard constraints.
...
- **Fundamentals for all 145 tickers:** market cap, trailing P/E, forward P/E, revenue growth, profit margins, debt/equity, ROE
  ...
  "actions": [
    {
      ...
      "thesis": "One-sentence edge. • Momentum: ... • Technicals: ... • Fundamentals: ... • Volume: ... • Sizing: ...",
      "stop_loss_pct": 0.06,
...
- Stop losses optional — Claude sets them at its discretion on any position where it wants downside protection
...
- **Single cron:** `0 21 * * 1-5` UTC — 5:00 PM EDT / 4:00 PM EST (DST-proof, never needs updating)
- **Swing job** runs first, then **long-term job** runs after it completes (sequential, no git conflicts)

Both jobs run sequentially to avoid git push conflicts. The workflow commits agent output files back to the repo automatically.
